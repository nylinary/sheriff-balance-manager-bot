...
def history_keyboard(
    ...
    for op in operations:
        ...
        label = (
            f"#{op.operation_id} | {uname} | {format_amount(op.amount)} | {cur_title}"
        )
        if op.is_reverted:
            label = f"↩️ {label}"
        rows.append(
            [InlineKeyboardButton(text=label, callback_data=f"h:op:{op.operation_id}")]
        )
    ...
    if page > 1:
        nav.append(
            InlineKeyboardButton(text="⬅️", callback_data=f"h:p:{page - 1}{suffix}")
        )
    else:
    ...
    nav.append(
        InlineKeyboardButton(
            text=f"{page}/{total_pages}", callback_data=f"h:goto{suffix}"
        )
    )
